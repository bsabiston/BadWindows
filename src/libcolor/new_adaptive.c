...
int x, y;
{
  int stwidth, stheight, string_width(), *draw_adapt_box(), *control_foreground_contrast(), *b_rgb, *new_f_rgb, *old_f_rgb;
...
int width, height, x, y;
{
  ...
  set_text_clip_rectangle (x-3, y-3, x+width+6, y+height+6);
...
int x, y, ht, wd;
{
  ...
  if (ht == 0) { printf ("\nfind_avarage_bkgrd_color() in adaptive.c  OOooops! height = 0 can't do it"); exit(1); }
    ...
    if ((R = (unsigned char *) malloc (sizeof (unsigned char)*ht*wd)) == NULL) printf("\nERROR find_average_bkgrd_color malloc failed R, adaptive.c");
    if ((G = (unsigned char *) malloc (sizeof (unsigned char)*ht*wd)) == NULL) printf("\nERROR find_average_bkgrd_color malloc failed G, adaptive.c");
    if ((B = (unsigned char *) malloc (sizeof (unsigned char)*ht*wd)) == NULL) printf("\nERROR find_average_bkgrd_color malloc failed B, adaptive.c");

    if ((RGB = (int *) malloc (sizeof (int)*3)) == NULL) printf("\nERROR find_average_bkgrd_color malloc failed RGB, adaptive.c");
...
int *RGB;
{
  ...
  *rgb = (float) (*RGB/255.0);    *(rgb+1) = (float) (*(RGB+1)/255.0);    *(rgb+2) = (float) (*(RGB+2)/255.0);
