...
#indef USESTARBASE
static unsigned char *mback= NULL, *mlinebuf= NULL;
static int mbacksize = 0, mlinesize = 0;
#endif
...
static int gridsize = 12,linestat = 1,updatestat = 1, zoomstat = 0;
...
#indef USESTARBASE
    ...
    if (mbacksize < wide*tall) {
      if (mback) free(mback);
      mback = malloc(wide*tall*3);
      mbacksize = wide*tall;
      if (!mback){
	printf("ERROR mallocing palette resize\n");
	mbacksize = wide = tall = 0;
      }
    }
    ...
    /* DBUFFER NOTE: this little code block sets flags for the right db bank */
    /* also a line buffer is allocated if necessary */
    pmask = 0xFFFFFFFF;
    shifter = 0;

    if (W->display->bits & DBUFFER) {

      /* get line buffer memory reserved if necessary */
      if ((mlinesize/gridsize) < MAX(wide,tall))
	{
	  if (mlinebuf) free(mlinebuf);
	  mlinebuf = (unsigned char *) malloc((MAX(wide,tall)*gridsize)+1);  /* +1 is for extra black line at the end */
	  mlinesize = (MAX(wide,tall)*gridsize)+1;
	  if (!mlinebuf){
	    printf("ERROR mallocing palette line buffer\n");
	    mlinebuf = NULL;
	    mlinesize = 0;
	  }
	}

      if (W->display->bits & WBANK) shifter = 0; 
      else shifter = 4;
      pmask = 0x000000F0;

      bred = (BRED&pmask)>>shifter;
      bgreen = (BGREEN&pmask)>>shifter;
      bblue = (BBLUE&pmask)>>shifter;
    }
    else bred = BRED, bgreen = BGREEN, bblue = BBLUE;
	...
	for (x = 0; x<wide; x++)
	  {
	    ...
	    if ((stat)&&(W->display->bits & DBUFFER))
	      {
		fill_color(screen,FCON(r),FCON(g),FCON(b));
		dcrectangle(screen,W->x1+x*gridsize+1,W->y1+y*gridsize+1,W->x1+(x+1)*gridsize-1,W->y1+(y+1)*gridsize-1);
	      }

	    if (stat) {
	      *buffer = (r&pmask)>>shifter;
	      *(buffer+wide*tall) = (g&pmask)>>shifter;
	      *(buffer+wide*tall*2) = (b&pmask)>>shifter;
	    }
	    else  {
	      *buffer = bred;
	      *(buffer+wide*tall) = bgreen;
	      *(buffer+wide*tall*2) = bblue;
	    }
...
/*      bank_switch(W->display->fildes,2,0);
      dcblock_write(W->display->fildes,W->x1,W->y1,wide,tall,mback,FALSE);
      BrainDamagedFastMag(screen,mlinebuf,startx,starty,wide,tall,W->x1,W->y1,gridsize,linestat,BRED,BGREEN,BBLUE); 
      bank_switch(W->display->fildes,1,0);
      dcblock_write(W->display->fildes,W->x1,W->y1,wide,tall,mback+wide*tall,FALSE);
      BrainDamagedFastMag(screen,mlinebuf,startx,starty,wide,tall,W->x1,W->y1,gridsize,linestat,BRED,BGREEN,BBLUE);
      bank_switch(W->display->fildes,0,0);
      dcblock_write(W->display->fildes,W->x1,W->y1,wide,tall,mback+wide*tall*2,FALSE);
      BrainDamagedFastMag(screen,mlinebuf,startx,starty,wide,tall,W->x1,W->y1,gridsize,linestat,BRED,BGREEN,BBLUE);
*/
    } 
    else {
