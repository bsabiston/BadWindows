     ...
     int val;
{
  ...
  if (!(screen = OpenRenaissance("/dev/crt", "hpA1439A", CRX24_FLAGS, val)))
    ...
    if (!(screen = OpenRenaissance("/dev/crt", "hp98765", LAME_FLAGS, val)))
...
struct OutDev *Open750(string1, string2, bits, val)     /* v 1.0b */
     char *string1, *string2;
     int val, bits;
{
  struct OutDev *screen = NULL;
  ...
  printf("trying the 750\n",string1,string2);
  printf("gopen %s %s\n",string1,string2);
  if ((display = gopen(string1, OUTDEV, string2, flags)) != -1)
    {
      printf("gopen was successful\n");
      screen = (struct OutDev *)malloc(sizeof(struct OutDev));
      screen->bits = bits;
      screen->width  = 2048;
      screen->viswidth = 1280;
     ...
     int val, bits;
{
  ...
  setbuf( stdout, NULL);
  if (val) flags = INIT|THREE_D|MODEL_XFORM|RESET_DEVICE;
  else flags = THREE_D|MODEL_XFORM|RESET_DEVICE;

  printf("gopen %s %s\n",string1,string2);
  if ((display = gopen(string1, OUTDEV, string2, flags)) != -1)
    {
      printf("successful gopen\n");
      screen = (struct OutDev *)malloc(sizeof(struct OutDev));
      screen->bits = bits;
      screen->width  = 2048;
      screen->viswidth = 1280;
...
struct OutDev *OpenCrx8X(string1, string2, bits, val)     /* v 1.0b */
     char *string1, *string2;
     int val, bits;
{
  struct OutDev *screen = NULL;
